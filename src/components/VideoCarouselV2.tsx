...
export async function VideoCarouselV2() {
  ...
  return (
    <Card className="p-6 shadow-soft bg-gradient-card">
      ...
      <div className="flex flex-col">
        <h2 className="text-xl font-semibold flex items-center gap-2">
          User stats and evaluations
        </h2>
        <span>
          Username: <pre>{chosenUser.username}</pre>
        </span>
        <span>
          Content profile: <pre>{JSON.stringify(chosenUser.content_profile, null, 2)}</pre>
        </span>
        <span>
          Content analysis: <pre>{JSON.stringify(chosenUser.content_analysis, null, 2)}</pre>
        </span>
        <span>
          Profile analysis: <pre>{JSON.stringify(chosenUser.profile_analysis, null, 2)}</pre>
        </span>
        <span>
          Recommendations: <pre>{chosenUser.recommendation ? JSON.stringify(chosenUser.recommendation, null, 2) : "No recommendations available"}</pre>
        </span>
      </div>
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  ...
                  <div className="flex flex-col mt-2 text-xs text-muted-foreground">
                    <span>Engagement rate: {video.engagement_rate}</span>
                    <span>Amplification rate: {video.amplification_rate}</span>
                    <span>Engagement score: {video.engagement_score}</span>
                    <span>Reach norm: {video.reach_norm}</span>
                    <br />
                    <span>Engagement rating: {video.engagement_rating}</span>
                    <span>Video evaluation: {video.evaluation}</span>
                    <br />
                    <span>AI Processed Output's Input: 
                      <pre className="whitespace-pre-wrap">
                        {JSON.stringify(video.ai_processed_output.input, null, 2)}
                      </pre>
                    </span>
                    <span>AI Processed Output's Step 0 Output: 
                      <pre className="whitespace-pre-wrap">
                        {JSON.stringify(video.ai_processed_output.stage0_output, null, 2)}
                      </pre>
                    </span>
                    <span>AI Processed Output's Step 1 Output: 
                      <pre className="whitespace-pre-wrap">
                        {JSON.stringify(video.ai_processed_output.stage1_output, null, 2)}
                      </pre>
                    </span>
                    <span>AI Processed Output's Step 2 Output: 
                      <pre className="whitespace-pre-wrap">
                        {JSON.stringify(video.ai_processed_output.stage2_output, null, 2)}
                      </pre>
                    </span>
                    <span>AI Processed Output's Step 3 Output: 
                      <pre className="whitespace-pre-wrap">
                        {JSON.stringify(video.ai_processed_output.stage3_output, null, 2)}
                      </pre>
                    </span>
                  </div>
                </div>

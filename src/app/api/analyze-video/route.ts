...
export async function POST(req: Request) {
  ...
  try {
    ...
    const perVidAnalytic = videoDataProfileData.map(
      (video) => video.ai_processed_output["stage3_output"]
    )
    ...
    const stats = perVidAnalytic.reduce(
      (acc, analytic) => {
        ...
        acc.primaryFrequency[content_type_primary] =
          (acc.primaryFrequency[content_type_primary] || 0) + 1
        acc.secondaryFrequency[content_type_secondary] =
          (acc.secondaryFrequency[content_type_secondary] || 0) + 1

        // Sum confidences for averaging later
        acc.primaryConfidenceSum[content_type_primary] =
          (acc.primaryConfidenceSum[content_type_primary] || 0) +
          confidence_primary
        acc.secondaryConfidenceSum[content_type_secondary] =
          (acc.secondaryConfidenceSum[content_type_secondary] || 0) +
          confidence_secondary

        // Track total counts for averaging confidences
        acc.primaryCount[content_type_primary] =
          (acc.primaryCount[content_type_primary] || 0) + 1
        acc.secondaryCount[content_type_secondary] =
          (acc.secondaryCount[content_type_secondary] || 0) + 1
    ...
    let content_type_primary = Object.keys(stats.primaryFrequency).reduce(
      (a, b) => (stats.primaryFrequency[a] > stats.primaryFrequency[b] ? a : b)
    )
    let content_type_secondary = Object.keys(stats.secondaryFrequency).reduce(
      (a, b) =>
        stats.secondaryFrequency[a] > stats.secondaryFrequency[b] ? a : b
    )

    // Compute average confidences
    const confidence_primary =
      stats.primaryConfidenceSum[content_type_primary] /
      stats.primaryCount[content_type_primary]
    const confidence_secondary =
      stats.secondaryConfidenceSum[content_type_secondary] /
      stats.secondaryCount[content_type_secondary]

    // Swap primary and secondary if confidences are within 0.1
    if (Math.abs(confidence_primary - confidence_secondary) <= 0.1) {
      ;[content_type_primary, content_type_secondary] = [
        content_type_secondary,
        content_type_primary,
      ]
    }
    ...
    // Compute consistency
    const consistency =
      perVidAnalytic.filter(
        (analytic) => analytic.content_type_primary === content_type_primary
      ).length / perVidAnalytic.length
    ...
    if (updateResult.error) {
      ...
      console.log(
        "Successfully updated user content profile:",
        updateResult.data
      )
    }
